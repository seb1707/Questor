    ...
    public static class Salvage
    {
        ...
        private static void ActivateTractorBeams()
        {
            ...
            for (int i = tractorBeams.Count - 1; i >= 0; i--)
            {
                ...
                if (Settings.Instance.DebugTractorBeams) Logging.Log("Salvage.ActivateTractorBeams", "MyShip.Velocity [" + Math.Round(Cache.Instance.MyShipEntity.Velocity, 0) + "]", Logging.Teal);
        ...
        private static void TargetWrecks()
        {
            ...
            foreach (EntityCache wreck in wrecks.Where(w => !w.IsIgnored))
            {
                ...
                {
                    if (Settings.Instance.DebugTargetWrecks) Logging.Log("Salvage.TargetWrecks", "Debug: if (wreck.GroupId != (int)Group.Wreck && wreck.Distance < (int)Distances.SafeScoopRange)", Logging.Teal);
                    continue;
                }
                ...
                if (!Cache.Instance.SalvageAll)
                {
                    ...
                    {
                        //if (Settings.Instance.DebugTargetWrecks) Logging.Log("Salvage.TargetWrecks", "Debug: if (Settings.Instance.WreckBlackList.Any(a => a == wreck.TypeId) && (wreck.IsWreckEmpty || wreck.Distance < (int)Distances.SafeScoopRange))", Logging.Teal);
                        continue;
                    }
                }
