...
namespace Questor.Modules.EVEInteration
{
    #region Anomalies
    internal class DirectAnomalies
    {
        ...
        internal void WarpToAnomaly(int distance)
        {
            ...
            {
                Cache.Instance.DirectEve.ThreadedLocalSvcCall("menu", "WarpToScanResult", Id, distance);
            }
        }
        ...
        internal void WarpToAnomaly()
        {
            if (!string.IsNullOrEmpty(Id) && SignalStrength == 1)
            {
                Cache.Instance.DirectEve.ThreadedLocalSvcCall("menu", "WarpToScanResult", Id);
            }
        }
        ...
        internal void WarpFleetToAnomaly(int distance)
        {
            if (!string.IsNullOrEmpty(Id) && SignalStrength == 1)
            {
                Cache.Instance.DirectEve.ThreadedLocalSvcCall("menu", "WarpFleetToScanResult", Id, distance);
            }
        }
        ...
        internal void WarpFleetToAnomaly()
        {
            if (!string.IsNullOrEmpty(Id) && SignalStrength == 1)
            {
                Cache.Instance.DirectEve.ThreadedLocalSvcCall("menu", "WarpFleetToScanResult", Id);
            }
        }
    }
