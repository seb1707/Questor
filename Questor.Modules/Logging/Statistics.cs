    ...
    public class Statistics
    {
        ...
        public static bool LogAllEntities(List<EntityCache> things, bool force = false)
        {
            ...
            if (things.Any())
            {
                ...
                {
                    icount++;
                    Logging.Log("[" + icount + "]", thing.Name + "[" + Math.Round(thing.Distance / 1000, 0) + "] GroupID [" + thing.GroupId + "] ID[" + Cache.Instance.MaskedID(thing.Id) + "] IsHVT [" + thing.IsHighValueTarget + "] IsLVT [" + thing.IsLowValueTarget + "] IsIgnored [" + thing.IsIgnored + "] IsIgnoredrefreshes [" + thing.IsIgnoredRefreshes + "]", Logging.Debug);
                }
            }
