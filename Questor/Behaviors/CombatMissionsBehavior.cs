...
namespace Questor.Behaviors
{
    public class CombatMissionsBehavior
    {
        ...
        private bool WeShouldBeInStationAndOutOfSessionChange()
        {
            ...
            {
                if (Logging.DebugWeShouldBeInSpaceORInStationAndOutOfSessionChange) Logging.Log("Debug1", "if (!Cache.Instance.InSpace && !Cache.Instance.InStation)", Logging.Debug);
            ...
            {
                if (Logging.DebugWeShouldBeInSpaceORInStationAndOutOfSessionChange) Logging.Log("Debug1", "if (Cache.Instance.InStation)", Logging.Debug);
                if (DateTime.UtcNow < Time.Instance.LastInSpace.AddSeconds(16))
                {
                    if (Logging.DebugWeShouldBeInSpaceORInStationAndOutOfSessionChange) Logging.Log("Debug1", "if (DateTime.UtcNow < Time.Instance.LastInSpace.AddSeconds(16))", Logging.Debug);
                    //wait... we must be in a session change
                    return false;    
                }
        ...
        private bool WeShouldBeInSpaceAndOutOfSessionChange()
        {
            ...
            {
                //wait... we must be in a session change
                if (Logging.DebugWeShouldBeInSpaceORInStationAndOutOfSessionChange) Logging.Log("Debug2", "if (!Cache.Instance.InSpace && !Cache.Instance.InStation)", Logging.Debug);
                return false;
            }

            if (Cache.Instance.InSpace)
            {
                if (Logging.DebugWeShouldBeInSpaceORInStationAndOutOfSessionChange) Logging.Log("Debug2", "if (Cache.Instance.InSpace)", Logging.Debug);
                if (DateTime.UtcNow < Time.Instance.LastInStation.AddSeconds(16))
                {
                    if (Logging.DebugWeShouldBeInSpaceORInStationAndOutOfSessionChange) Logging.Log("Debug2", "if (DateTime.UtcNow < Time.Instance.LastInStation.AddSeconds(16))", Logging.Debug);
                    //wait... we must be in a session change
                    return false;
                }
            ...
            if (Logging.DebugWeShouldBeInSpaceORInStationAndOutOfSessionChange) Logging.Log("Debug2", "Are we not in space?! InSpace[" + Cache.Instance.InSpace + "] InStation[" + Cache.Instance.InStation + "]", Logging.Debug);
            return false;
        }
        ...
        private void IdleCMBState()
        {
            ...
            {
                if (Logging.DebugAutoStart || Logging.DebugIdle) Logging.Log("CombatMissionsBehavior", "DebugAutoStart: Autostart [" + Settings.AutoStart + "]", Logging.White);
                ...
                if (Settings.RandomDelay > 0 || Settings.MinimumDelay > 0)
                {
                    _randomDelay = (Settings.RandomDelay > 0 ? _random.Next(Settings.RandomDelay) : 0) + Settings.MinimumDelay;
                    LastCMBAction = DateTime.UtcNow;
            ...
            if (Logging.DebugAutoStart) Logging.Log("CombatMissionsBehavior", "DebugIdle: Autostart is currently [" + Settings.AutoStart + "]", Logging.White);
        ...
        private void StartCMBState()
        {
            ...
            if (_States.CurrentAgentInteractionState == AgentInteractionState.Idle)
            {
                ...
                if (Settings.EnableStorylines && _storyline.HasStoryline())
                {
            ...
            if (_States.CurrentAgentInteractionState == AgentInteractionState.Done)
            {
                MissionSettings.UpdateMissionName(AgentID);
                _States.CurrentAgentInteractionState = AgentInteractionState.Idle;
            ...
            if (_States.CurrentArmState == ArmState.Idle)
            {
                ...
                else
                {
                    ...
                    if (Combat.WeaponGroupId != 53) //civilian guns of all types
                    {
                        ...
                        //
                        Arm.AmmoToLoad.AddRange(MissionSettings.AmmoToLoad);
                    }
                }
            }
        ...
        {
            if (!string.IsNullOrEmpty(Settings.UndockBookmarkPrefix))
            {
                IEnumerable<DirectBookmark> warpOutBookmarks = Cache.Instance.BookmarksByLabel(Settings.UndockBookmarkPrefix ?? "");
        ...
        private void GotoBaseCMBState()
        {
            ...
            {
                if (Logging.DebugGotobase) Logging.Log("CombatMissionsBehavior", "GotoBase: if (Settings.AvoidBumpingThings)", Logging.White);
            ...
            if (_States.CurrentTravelerState == TravelerState.AtDestination && Cache.Instance.InStation && DateTime.UtcNow > Time.Instance.LastInSpace.AddSeconds(Cache.Instance.RandomNumber(10, 15))) // || DateTime.UtcNow.Subtract(Cache.Instance.EnteredCloseQuestor_DateTime).TotalMinutes > 10)
            {
                ...
                else if (_States.CurrentCombatState != CombatState.OutOfAmmo && MissionSettings.Mission != null && MissionSettings.Mission.State == (int)MissionState.Accepted)
                {
                    ValidateCombatMissionSettings();
        ...
        private void CompleteMissionCMBState()
        {
            ...
            if (_States.CurrentAgentInteractionState == AgentInteractionState.Done)
            {
                ...
                else if (Statistics.LastMissionCompletionError.AddSeconds(10) < DateTime.UtcNow)
                {
        ...
        private void StatisticsCMBState()
        {
            ...
            if (Statistics.MissionStats1Log || Statistics.MissionStats3Log || Statistics.MissionStats3Log)
            {
                try
                {
                    ...
                    //Logging.Log("CombatMissionsBehavior.Idle", "Settings.CombatShipName.ToLower() [" + Settings.CombatShipName.ToLower() + "]", Logging.Teal);
                ...
                {
                    Logging.Log("CombatMissionsBehavior.Idle", "if (Cache.Instance.ActiveShip != null && Cache.Instance.ActiveShip.GivenName.ToLower() == Settings.CombatShipName.ToLower())", Logging.Teal);
                }
            }
        ...
        private void GotoMissionCMBState()
        {
            ...
            if (missionDestination == null || missionDestination.AgentId != AgentID) // We assume that this will always work "correctly" (tm)
            {
                ...
                Traveler.Destination = new MissionBookmarkDestination(MissionSettings.GetMissionBookmark(AgentID, nameOfBookmark));
            ...
            if (_States.CurrentUnloadLootState == UnloadLootState.Done)
            {
                ...
                if (Salvage.AfterMissionSalvaging)
                {
                    if (Cache.Instance.GetSalvagingBookmark == null)
                    {
                        ...
                        //if (Settings.CharacterMode == "Salvager")
                        //{
                    ...
                    {
                        Logging.Log("CombatMissionsBehavior.Unloadloot", "There are [ " + Cache.Instance.BookmarksByLabel(Settings.BookmarkPrefix + " ").Count + " ] more salvage bookmarks left to process", Logging.White);
                        ...
                        else //begin after mission salvaging now, rather than later
                        {
                            if (_States.CurrentQuestorState == QuestorState.DedicatedBookmarkSalvagerBehavior)
                            {
                                Logging.Log("CombatMissionsBehavior.Unloadloot", "CharacterMode: [" + Settings.CharacterMode + "], AfterMissionSalvaging: [" + Salvage.AfterMissionSalvaging + "], CombatMissionsBehaviorState: [" + _States.CurrentCombatMissionBehaviorState + "]", Logging.White);
                                _States.CurrentCombatMissionBehaviorState = CombatMissionsBehaviorState.CheckBookmarkAge;
                                Statistics.StartedSalvaging = DateTime.UtcNow;
                            }
                            else
        ...
        private void BeginAftermissionSalvagingCMBState()
        {
            ...
            if (DateTime.UtcNow.Subtract(_lastSalvageTrip).TotalMinutes < Time.Instance.DelayBetweenSalvagingSessions_minutes && Settings.CharacterMode.ToLower() == "salvage".ToLower())
            {
            ...
            if (_States.CurrentArmState == ArmState.Done)
            {
                ...
                if (bookmark == null && Cache.Instance.BookmarksByLabel(Settings.BookmarkPrefix + " ").Any())
                {
                    bookmark = Cache.Instance.BookmarksByLabel(Settings.BookmarkPrefix + " ").OrderBy(b => b.CreatedOn).FirstOrDefault();
                    if (bookmark == null)
                    {
                        _States.CurrentCombatMissionBehaviorState = CombatMissionsBehaviorState.Idle;
                        return;
        ...
        private void SalvageCMBState()
        {
            ...
            if (deadlyNPC != null)
            {
                ...
                List<DirectBookmark> missionSalvageBookmarks = Cache.Instance.BookmarksByLabel(Settings.BookmarkPrefix + " ");
        ...
        private bool CMBEveryPulse()
        {
            ...
            if (Settings.FinishWhenNotSafe && (_States.CurrentCombatMissionBehaviorState != CombatMissionsBehaviorState.GotoNearestStation))
            {
                //need to remove spam
        ...
        public void ProcessState()
        {
            ...
            switch (_States.CurrentCombatMissionBehaviorState)
            {
                ...
                case CombatMissionsBehaviorState.PrepareStorylineGotoBase:
                    ...
                    if (NavigateOnGrid.AvoidBumpingThingsBool)
                    {
                        if (Logging.DebugGotobase) Logging.Log("CombatMissionsBehavior", "PrepareStorylineGotoBase: if (Settings.AvoidBumpingThings)", Logging.White);
