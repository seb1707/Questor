...
namespace D3DDetour
{
	public class D3D9 : D3DHook
	{
		...
		public override void Initialize()
		{
			using (var d3d = new Direct3D())
			{
				using (var tmpDevice = new Device(d3d, 0, DeviceType.Hardware, IntPtr.Zero, CreateFlags.HardwareVertexProcessing, new PresentParameters() { BackBufferWidth = 1, BackBufferHeight = 1 }))
				{
					...
					EndScenePointer = (IntPtr)Marshal.ReadInt32((IntPtr)((int)(IntPtr)Marshal.ReadInt32(tmpDevice.ComPointer) + (VMT_ENDSCENE*4)));
					ResetPointer = (IntPtr)Marshal.ReadInt32((IntPtr)((int)(IntPtr)Marshal.ReadInt32(tmpDevice.ComPointer) + (VMT_RESET*4)));
				}
			}
			
			_endSceneHook = LocalHook.Create((IntPtr)EndScenePointer, new Direct3D9EndScene(Callback), this);
			_endSceneHook.ThreadACL.SetExclusiveACL(new Int32[] { 1 });
		...
		{
			RaiseEvent();
			  using (Device d3d = Device.FromPointer(device))
            {
