...
namespace Questor
{
    ...
    public partial class QuestorUI : Form
    {
        ...
        private void UpdateUiTick(object sender, EventArgs e)
        {
            ...
            if (DateTime.UtcNow.Subtract(Time.Instance.LastFrame).TotalSeconds > (Time.Instance.NoFramesRestart_seconds + extraWaitSeconds) && DateTime.UtcNow.Subtract(Program.EVEAccountLoginStarted.Value).TotalSeconds > 300)
            {
            ...
            if (DateTime.UtcNow.Subtract(Time.Instance.LastSessionIsReady).TotalSeconds > (Time.Instance.NoSessionIsReadyRestart_seconds + extraWaitSeconds) &&
                    DateTime.UtcNow.Subtract(Program.EVEAccountLoginStarted.Value).TotalSeconds > 210)
            {
